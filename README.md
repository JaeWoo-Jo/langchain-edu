...
**해결** (`agent/app/agents/search_agent.py`, `agent/app/agents/tools/search_price.py`):
...
| recursion_limit | **40회** | LangGraph 전체 노드 실행 횟수 상한, 초과 시 LLM 폴백 응답 |
...
| 순서 | 질문 | 트리거되는 장치 |
|------|------|---------------|
| Q1 | 장 보러 가는데 요즘 뭐가 제일 싸? | Planner 카테고리→품목 분해, search_price x4 |
| Q2 | 감자랑 양파 가격 비교해줘 | 다품목 분리 검색, compare_prices 테이블 |
| Q3 | 양파 가격 추이 차트 보여줘 | create_price_chart 차트 렌더링 |
| Q4 | 양파로 뭐 해먹을 수 있어? | search RAG 통합검색 |

### Q1. "장 보러 가는데 요즘 뭐가 제일 싸?"
